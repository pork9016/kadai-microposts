class User < ApplicationRecord
  ...
  self.likes.find_or_create_by(favorite_id: other_microposts.id)
  end  
  
  def unfavorite(other_microposts)
   like = self.likes.find_or_create_by(favorite_id: other_microposts.id)
    like.destroy if like
  end
  
  def liking?(other_microposts)
  self.favorites.include?(other_microposts)  
  end
